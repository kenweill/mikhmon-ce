...
### 🔧 Fixed
- **`settings/settings.php`** — Session name is now forced to lowercase and restricted to letters and numbers only. JS validator silently converts input to lowercase as the user types, then alerts and clears if spaces or special characters are detected. PHP sanitization changed from converting spaces to hyphens to fully stripping anything outside `[a-z0-9]`. Reserved name list simplified to lowercase-only since input is always lowercase. Fixes a major bug where session names with hyphens, mixed case, or special characters caused broken connectivity, inability to edit/delete router profiles, and general config corruption
- **`settings/settings.php`** — Fixed phantom `new-XXXX` entries being written to `config.php` every time the Add Router button was clicked. Root cause was the new-router block writing a placeholder entry immediately on page load before any details were entered. Rewrote the approach: no placeholder is ever written to config — clicking Add Router now just redirects to the settings form. On Save, a new router is detected by checking if `$session` starts with `new-` and writes directly to config using the actual session name the user typed. Existing router edits continue to use the original `str_replace` rename approach
- **`settings/settings.php`** — Fixed infinite redirect loop when opening the Add Router form. The `$currency == ""` guard was firing because `$data[$session]` doesn't exist for a new session, causing it to redirect back to itself endlessly. Fixed by adding `explode("-", $session)[0] != "new"` to the guard condition
- **`settings/settings.php`** — Auto Load, Idle Timeout, and Traffic Interface now default to `10`, `10`, and `1` respectively when adding a new router, matching the original placeholder defaults that were previously written to config
- **`settings/settings.php`** — Added `placeholder="e.g. Rp, Php, USD"` to the Currency input field so users know the expected format when adding a new router
- **`include/menu.php`** — Fixed wrong sidebar menu showing on the Add Router page. Dashboard, Session Settings, Upload Logo, and Template Editor were appearing before any router was saved. Two root causes: (1) `$session == "new"` never matched `new-XXXX` — fixed using `explode("-", $session)[0] == "new"`; (2) same stale check in the active-state block higher up in the file — fixed the same way
