...
- **`settings/settings.php`** — Fixed two related bugs when adding a new router. (1) Clicking "Add Router" immediately wrote a placeholder `new-XXXX` entry to `config.php` before the user filled in any details or clicked Save — caused by the new-router block having no form submission check. (2) Because the placeholder was written on every page load including the Save submission, saving a new router wrote the entry twice, resulting in duplicate sessions in the router list. Fixed by adding `!isset($_POST['save'])` guard to the new-router block so the placeholder is only written on initial page open, not on form submission
