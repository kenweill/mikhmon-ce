...
if (!isset($_SESSION["mikhmon"])) {
  ...
  if (isset($_POST['name'])) {
    ...
    // ROS6 & ROS7 compatible on-login and bgservice scripts
    if ($expmode == "rem") {
      $onlogin = mikhmon_build_onlogin($expmode, $price, $validity, $sprice, $getlock, $name);
      $mode = "remove";
    } elseif ($expmode == "ntf") {
      $onlogin = mikhmon_build_onlogin($expmode, $price, $validity, $sprice, $getlock, $name);
      ...
      $onlogin = ':put (",,' . $price . ',,,noexp,' . $getlock . ',")' . ($getlock == "Enable" ? '; [:local mac $"mac-address"; /ip hotspot user set mac-address=$mac [find where name=$user]]' : '');
      $mode = "";
    } else {
      $onlogin = "";
      $mode = "";
    ...
    $bgservice = mikhmon_build_bgservice($name, $mode);
